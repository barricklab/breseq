...
DIFF_BIN=`which diff`
DIFF_IGNORE='^(#=COMMAND|#=CREATED|#=PROGRAM|#=READSEQ|#=REFSEQ)'
	...
	for (( i=0; i<${#EXPECTED_OUTPUTS[@]}; i++ )); do
		...
		if [[ "$?" -ne 0 || $CHK ]]; then
			...
			if [[ -n "${REFERENCE_ARG+1}" ]]; then
				echo ${GDTOOLS} COMPARE ${REFERENCE_ARG} ${COMPARE_ARG} -o ${SELF}/failed_compare.html "${CURRENT_OUTPUTS[$i]}" "${EXPECTED_OUTPUTS[$i]}"
				${GDTOOLS} COMPARE ${REFERENCE_ARG} ${COMPARE_ARG} -o ${SELF}/failed_compare.html "${CURRENT_OUTPUTS[$i]}" "${EXPECTED_OUTPUTS[$i]}"
			fi
		else
