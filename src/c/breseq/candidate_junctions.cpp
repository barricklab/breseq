    ...
    while (stitched_again && (alignments.size() > 1))
    {
      ...
      for (size_t i = 0; i < ordered.size(); i++)
      {
        for (size_t j = i + 1; j < ordered.size(); j++)
        {
          ...
          // Reject pairs whose query ranges overlap by half or more of their combined span --
          // a genuine natural split (two complementary pieces around one small ambiguous-repeat
          // indel) should mostly not overlap, while a decoy pair (e.g. two near-full-length
          // placements of the same read against different copies of a repeat) routinely does.
          // Without this, such a pair can reach the mismatch-correction geometry math below and
          // produce an internally inconsistent breakpoint. overlap <= 0 means a true gap (no
          // overlap at all), which always passes.
          int32_t overlap = static_cast<int32_t>(a_end) - static_cast<int32_t>(b_start) + 1;
