                                         ...
                                         
{
    ...
    vector< pair<uint32_t, uint32_t> > all_search_pairs_vector_for_edging;
    ...
    vector< pair<uint32_t, uint32_t> > ranged_pairs_vector;
    vector< pair<uint32_t, uint32_t> > ranged_pairs_vector_copy;
    vector< pair<uint32_t, uint32_t> > ranged_pairs_vector_for_edging;
    ...
    */
    ...
    while ( all_search_pairs_vector.size() > 0 ) {
        ...
        if ( (double)(best_t_was_higher_counter) / randomizations >= .95 &&
         ...
         )
        {
            ...
            next_search_pair_by_tile.first = best_i_tile_hi + 1;
            next_search_pair_by_tile.second = best_j_tile_hi;
            ...
            if ( best_i_tile_hi + 1 > current_search_pair_by_tile.first )
            {
                ...
                next_search_pair_by_tile.second = best_i_tile_hi;
                all_search_pairs_vector.push_back( next_search_pair_by_tile );
            }
            ...
            if ( best_j_tile_hi < current_search_pair_by_tile.second )
            {
                next_search_pair_by_tile.first = best_j_tile_hi + 1;
        ...
        else
        {
            ...
            // Save current search pair as a range for later improved edge detection
            ranged_pairs_vector.push_back(current_search_pair_by_tile);
    ...
    // Copy ranged pairs vector for safekeeping
    ranged_pairs_vector_copy = ranged_pairs_vector;
     ...
     **
     **
     ** Time to apply improved edge detection to previously found outliers
     **
     **
     **/
    ...
    // Note: ranged_pairs_vector should have a size of at least 2 or 3 at
    ...
    /*
     * Load a new vector with search segments based on the edges of 
     * previously found ranges. This will be the search queue used by 
     * the new CBS algo loop for improved edge detection.
     */
    vector< pair<uint32_t, uint32_t> > ranged_pairs_vector_modified;
    pair<int32_t, int32_t> ranged_pair;
    pair<int32_t, int32_t> edging_pair;
    uint32_t edging_tile_size_multiplier = old_tile_size / tile_size;
    cout << "\nedging_tile_size_multiplier: " << edging_tile_size_multiplier << endl;
    
    while (ranged_pairs_vector.size() > 0)
    {
        ranged_pair.first = ranged_pairs_vector.back().first;
        ranged_pair.second = ranged_pairs_vector.back().second;
        ranged_pairs_vector.pop_back();
        
        // If the second tile in the "parent" ranged pair is the last tile in genome,
        // ignore it, because it contains lots of noise, and we can't search on the outside
        // of the first or last tiles. (We may add the first and last tile to the queue
        // anyway, but commented out for now, see below.)
        if (ranged_pair.second == (num_large_tiles_in_genome - 1))
        {
            // Debugging output
            cout << "\nranged_pair.first: " << ranged_pair.first;
            cout << "\nranged_pair.second: "<< ranged_pair.second << "\n\n";
            
            continue;
        }
        
        // Else, add the appropriate range based on new tile size to the queue
        edging_pair.first = ranged_pair.second * edging_tile_size_multiplier - edging_tile_size_multiplier + 1;
        
        edging_pair.second = ranged_pair.second * edging_tile_size_multiplier + edging_tile_size_multiplier;
        
        all_search_pairs_vector_for_edging.push_back(edging_pair);
    ...
    /*
     * Note: Uncomment later if necessary, but for now I believe this creates too much noise,
     * so it's been commented out.
     *
     * Always add first and last tile in genome to search queue, because we don't know which
     * ranges are the actual outliers, so we need to check them all.
     */
    /*
    edging_pair.first = 1;
    edging_pair.second = edging_tile_size_multiplier;
    all_search_pairs_vector_for_edging.push_back(edging_pair);
    
    edging_pair.first = (num_large_tiles_in_genome - 1) * edging_tile_size_multiplier  - edging_tile_size_multiplier + 1;
    edging_pair.second = (num_large_tiles_in_genome - 1) * edging_tile_size_multiplier;
    all_search_pairs_vector_for_edging.push_back(edging_pair);
     */
    
    
    // Debugging output
    for (uint32_t i = 0; i < all_search_pairs_vector_for_edging.size(); i++)
    {
        cout << "\nedging_pairs_vector[i].first: " << all_search_pairs_vector_for_edging[i].first;
        cout << "\nedging_pairs_vector[i].second: " << all_search_pairs_vector_for_edging[i].second << "\n\n";
    }
    ...
    /*
     * Beginning of new CBS algorithm loop
     * for improved edge detection
     */
    
    // Until there are no more search segments in which to look for outliers...
    while ( all_search_pairs_vector_for_edging.size() > 0 ) {
        
        /* Set the current search segment: */
        
        // Remove the last search pair from the edging_pairs_vector and
        // set it as the current search pair. NOTE: using pop_back() is why
        // results are printed to the ranges and history files in seemingly
        // reverse order from which they are added to the queue (later down the line).
        current_search_pair_by_tile.first = all_search_pairs_vector_for_edging.back().first;
        current_search_pair_by_tile.second = all_search_pairs_vector_for_edging.back().second;
        all_search_pairs_vector_for_edging.pop_back();
        ...
        if (current_search_pair_by_tile.first == current_search_pair_by_tile.second ||
            current_search_pair_by_tile.first + 1 == current_search_pair_by_tile.second)
        {
            // Print to ranges file (TODO: DONT PRINT TILL AFTER IMPROVED EDGE DETECTION)
            ranges_file << ordered_positions_in_segment[current_search_pair_by_tile.first] - tile_size + 1 << "\t"
                    << ordered_positions_in_segment[current_search_pair_by_tile.second] << "\t"
                    << 0.0 << "\t"
                    << 1.0 << endl;
            ...
            // Save current search pair as a range for later improved edge detection
            ranged_pairs_vector_for_edging.push_back(current_search_pair_by_tile);
        ...
        // Set to largest possible negative number so the first t_score found
        // is always larger or smaller than the initialized value
        best_t_score_hi = -1.7e-308;
        best_t_score_lo = -1.7e-308;
        
        // Zero out values
        best_t_was_higher_counter = 0;
        best_t_was_lower_counter = 0;
        best_t_length_hi = 0;
        best_t_length_lo = 0;
             ...
             i_tile < current_search_pair_by_tile.second; i_tile++)
        {
                 ...
                 j_tile <= current_search_pair_by_tile.second; j_tile++)
            {
                ...
                // Keep track of subsegment with the highest t_score.
                // Note: Floats are being compared here, and thus approximate
                // equality is used. ex: abs(x - y) < 1.0e-16
                if ( t_score > best_t_score_hi ||
                    ( abs(t_score - best_t_score_hi) < 1.0e-16 && t_length > best_t_length_hi) )
                {
                    best_t_score_hi = t_score;
                    best_t_length_hi = t_length;
                    // Used for stepping thru the code in debug mode
                    best_i_tile_hi = i_tile;
                    best_j_tile_hi = j_tile;
                ...
                // Keep track of subsegment with the lowest t_score.
                // Note: Floats are being compared here, and thus approximate
                // equality is used. ex: abs(x - y) < 1.0e-16
                if ( t_score < best_t_score_lo ||
                    ( abs(t_score - best_t_score_lo) < 1.0e-16 && t_length > best_t_length_lo) )
                {
                    best_t_length_lo = t_length;
                    best_t_score_lo = t_score;
                    // Used for stepping thru the code in debug mode
                    best_i_tile_lo = i_tile;
                    best_j_tile_lo = j_tile;
                }
                
            } // End of j_tile for-loop
        } // End of i_tile for-loop
        
        // Save info about the potential outlier that has just been found, for debugging only.
        // The variables, but not these values, will be reused.
        i_position = ordered_positions_in_segment[best_i_tile_hi] - position_hash;
        j_position = ordered_positions_in_segment[best_j_tile_hi] - position_hash;
        sum_of_coverage_to_i_pos = sum_of_coverage_to_position[ i_position ];
        sum_of_coverage_to_j_pos = sum_of_coverage_to_position[ j_position ];
         ...
         * Note: Doing calculations for the left and right subsegments
         * may no longer be applicable. This code should be refactored
         * if this is the case. These also only account for
         * high-coverage outliers, not those for low-coverage.
         */
        ...
        for ( int32_t examinations = 0; examinations < randomizations; examinations++ )
        {
            ...
            // is always larger or smaller than the initialized value
            r_best_t_score_hi = -1.7e-308;
            r_best_t_score_lo = -1.7e-308;
            
            // Zero out values
            r_best_t_length_hi = 0;
            r_best_t_length_lo = 0;
                 ...
                 i_tile < current_search_pair_by_tile.second; i_tile++ )
            {
                     ...
                     j_tile <= current_search_pair_by_tile.second; j_tile++ )
                {
                    ...
                    // Determine number of tiles between i and j
                    r_t_length = j_tile - i_tile;
                    
                    // Keep track of subsegment with the highest t_score.
                    // Note: Floats are being compared here, and thus approximate
                    // equality is used. ex: abs(x - y) < 1.0e-16
                    if ( r_t_score > r_best_t_score_hi ||
                        ( abs(r_t_score - r_best_t_score_hi) < 1.0e-16 && r_t_length > r_best_t_length_hi))
                    {
                        r_best_t_score_hi = r_t_score;
                        r_best_t_length_hi = r_t_length;
                        // Used for stepping thru the code in debug mode
                        r_best_i_tile_hi = i_tile;
                        r_best_j_tile_hi = j_tile;
                    }
                    ...
                    // Note: Floats are being compared here, and thus approximate
                    // equality is used. ex: abs(x - y) < 1.0e-16
                    if ( r_t_score < r_best_t_score_lo ||
                        ( abs(r_t_score - r_best_t_score_hi) < 1.0e-16 && r_t_length > r_best_t_length_lo))
                    {
                        r_best_t_score_lo = r_t_score;
                        r_best_t_length_lo = r_t_length;
                        // Used for stepping thru the code in debug mode
                        r_best_i_tile_lo = i_tile;
                        r_best_j_tile_lo = j_tile;
                    }
            ...
            if (VERBOSE) {
                cout << "\n\n#####################################";
                cout << "\nbest_t_score_hi: " << fixed << setprecision(40) << best_t_score_hi;
                cout << "\nr_best_t_score_hi: " << fixed << setprecision(40) << r_best_t_score_hi;
                cout << "\nbest_t_score_lo: " << fixed << setprecision(40) << best_t_score_lo;
                cout << "\nr_best_t_score_lo: " << fixed << setprecision(40) << r_best_t_score_lo;
                cout << endl << endl;
            }
            ...
            /*
             * Count the times best_t_score_hi is higher than r_best_t_score_hi,
             * and best_t_score_lo is lower than r_best_t_score_lo.
             */
            ...
            if ( best_t_score_lo < r_best_t_score_lo )
            {
                best_t_was_lower_counter++;
            }
            ...
            /*  if (examinations - best_t_was_bigger_counter >= max_error_threshold){ ... */
            if ((examinations + 1) - best_t_was_higher_counter > max_error_threshold &&
                (examinations + 1) - best_t_was_lower_counter > max_error_threshold){
        ...
        cout << "\nbest_t_was_lower_counter = ";
        if ((double)(best_t_was_lower_counter) / randomizations >= .95)
            cout << "passed\n";
        else cout << "failed\n";
        ...
        // If the potential high-coverage outlier is significant, and if the
        // potential high-coverage outlier is NOT just the entire genome...
        if ( (double)(best_t_was_higher_counter) / randomizations >= .95 &&
            !((current_search_pair_by_tile.first == best_i_tile_hi + 1) && (current_search_pair_by_tile.second == best_j_tile_hi))
            )
        {
            ...
            // Add the middle subsegment (which is the newly found outlier between i and j)
            // to the search queue (i.e. all_search_pairs_vector_for_edging)
            next_search_pair_by_tile.first = best_i_tile_hi + 1;
            next_search_pair_by_tile.second = best_j_tile_hi;
            all_search_pairs_vector_for_edging.push_back( next_search_pair_by_tile );
                                          ...
                                          )
{
    ...
    /*
     * Merge ranges based on same copy number
     */
    add_to_merged_vector = true;
    while (range_t_vector.size() > 0)
    {
        cout << "\n-------------------------------" << endl;
        
        // Remove last range_t from vector
        range_t currentRange = range_t_vector.back();
        range_t_vector.pop_back();
        
        // Debugging output
        cout << "\ncurrentRange.first = " << currentRange.range_pair.first << endl;
        cout << "currentRange.second = " << currentRange.range_pair.second << endl;
        cout << "copy_number = " << currentRange.copy_number << endl << endl;
        
        // For each remaining candidate range within range_t_vector...
        // Note: We use <= because when the last range is removed from
        // the vector above, the vector size is 0, and we want this loop
        // to run one last time so we can make it to the final nested
        // else statement, so the final range will be added to the merged vector.
        for (uint32_t i = 0; i <= range_t_vector.size(); i++)
        {
            // Debug output
            cout << "\nrange_t_vector.size(): " << range_t_vector.size() << endl;
            
            range_t candidateRange = range_t_vector[i];
            
            // If the candidate range's end plus one is equal to the current
            // range's start, and if both ranges have the same copy number
            if (candidateRange.range_pair.second + 1 == currentRange.range_pair.first &&
                abs(candidateRange.copy_number - currentRange.copy_number) < 1.0e-8)
            {
                
                cout << "\nMade it into block 1" << endl;
                
                // Merge the two ranges into one by first removing both from the
                // range_t_vector... 
                range_t_vector.erase(range_t_vector.begin() + i);
                
                // ...then creating one range from them...
                range_t newRange;
                newRange.range_pair.first = candidateRange.range_pair.first;
                newRange.range_pair.second = currentRange.range_pair.second;
              ...
              // ...add that range back to the range_t_vector,
                ...
                // range_t_vector_merged yet because there might be more candidate
                // ranges left to merge with.
                add_to_merged_vector = false;
                break;
            }
                     ...
                     abs(candidateRange.copy_number - currentRange.copy_number) < 1.0e-8)
            {
                ...
                // ...then creating one range from them...
                range_t newRange;
                newRange.range_pair.first = currentRange.range_pair.first;
                newRange.range_pair.second = candidateRange.range_pair.second;
                newRange.copy_number = currentRange.copy_number;
                newRange.copy_number_float =
                    (currentRange.copy_number_float + candidateRange.copy_number_float) / 2.0;
            ...
            // Else, since candidate range coudn't be merged with current range,
            // and since this might be the last candidate to be examined, we
            // should let the current range be added to the range_t_vector_merged.
            // But there may in fact be more candidates to be examined, so we
            // don't break out of the for loop and instead let it terminate naturally.
            else
            {
                add_to_merged_vector = true;
            }
        ...
        // If there are no candidate ranges left to merge with the current range,
        // add it to the range_t_vector_merged
        if (add_to_merged_vector == true)
        {
            range_t_vector_merged.push_back(currentRange);
            
            // Debugging output
            cout << "\nrange_t_vector_MERGED:" << endl;
            cout << "range_pair.first = " << currentRange.range_pair.first << endl;
            cout << "range_pair.second = " << currentRange.range_pair.second << endl;
            cout << "copy_number = " << currentRange.copy_number << endl;
        }
        
    } // end of while-loop that merges ranges
    
    // Now all the merged ranges should be within range_t_vector_merged

    // Debug output
    /*
    for (uint32_t i = 0; i < range_t_vector_merged.size(); i++) {
        cout << "\nrange_t_vector_MERGED[" << i << "]:" << endl;
        cout << "range_pair.first = " << range_t_vector_merged[i].range_pair.first << endl;
        cout << "range_pair.second = " << range_t_vector_merged[i].range_pair.second << endl;
        cout << "copy_number = " << range_t_vector_merged[i].copy_number << endl;
    }
    cout << endl;
    */
     ...
     **
     **
     ** Print merged ranges to various output files
     **
     **
     **/
    
    // For each range within range_t_vector_merged
    for (uint32_t i = 0; i < range_t_vector_merged.size(); i++)
    {
        position_start = range_t_vector_merged[i].range_pair.first;
        position_end = range_t_vector_merged[i].range_pair.second;
